...
eq(find("nameMismatch").length, 0,
   "★コース名の食い違いでは警告しない（2026-08-18廃止。IDが一致すれば表記揺れは無視する）");
eq(W[0].level, "warn", "警告が先頭に並ぶ");
eq(find("stale").length, 0, "最新の記録が基準日と同じなら「止まっている」とは言わない");

/* コース名の食い違い警告は廃止した（2026-08-18 本人決定）。
   デルタは同じ講座でも書き方が日によって違い、頭8文字で救う作りでも
   C04・C08のように「省き方が頭から始まる」講座で誤検知が残ったため。
   ここでは「表記揺れでは警告が出ないこと」と「IDの取り違えは別の道で見つかること」を確かめる。 */

// 2026-08-17と2026-08-18に実際に誤検知が出たときの、本物の名前の組み合わせ
const REAL_PAIRS = [
  ...
  // ↓ 2026-08-18に誤検知した2本。省き方が「頭から」始まるので頭8文字では救えなかった
  ["C04", "【耳から学ぶビジネストレンド】人生100年時代。リスキリングなくして生き残れない。自ら未来を拓くアップデートの技術。",
          "【耳から学ぶ】人生100年時代のリスキリング"],
...
const realWarn = buildWarnings_(
  { rows: REAL_PAIRS.flatMap(p => [
      ...
      { date: "2026-08-18", id: p[0], name: p[2], cumEnroll: 200 }]) },
  REAL_PAIRS.map(p => ({ id: p[0], short: p[0] })), "2026-08-18");
eq(realWarn.length, 0,
   "★本物の台帳7コースぶんで警告が1件も出ない（C04・C08の誤検知も消える）");

/* IDを本当に取り違えたら、累計が大きく逆行するので必ず見つかる。
   コース名を見なくても、この道だけで取り違えは止められる。 */
const swapped = buildWarnings_({ rows: [
  { date: "2026-08-01", id: "C09", name: "教える技術", cumEnroll: 15020, cumRevenue: 48685 },
  { date: "2026-08-18", id: "C09", name: "開くリーダー", cumEnroll: 17841, cumRevenue: 67512 },
  { date: "2026-08-01", id: "C10", name: "開くリーダー", cumEnroll: 17841, cumRevenue: 67512 },
  { date: "2026-08-18", id: "C10", name: "教える技術", cumEnroll: 15020, cumRevenue: 48685 }
] }, COURSES_G, "2026-08-18");
eq(find2(swapped, "enrollDrop").filter(w => w.level === "warn").length, 1,
   "★C09とC10を入れ替えたら、累計登録の逆行として「警告」が出る");
eq(find2(swapped, "revenueDrop").filter(w => w.level === "warn").length, 1,
   "★累計収益の逆行としても「警告」が出る（名前を見なくても取り違えは止まる）");
